...
class PlayerProxy(object):
	...
	def chow(self, tile_list):
		...
		self.upTiles_opList.append(const.OP_CHOW)
	...
	def pong(self, tile):
		...
		self.upTiles_opList.append(const.OP_PONG)
	...
	def concealedKong(self, tile):
		...
		self.upTiles_opList.append(const.OP_CONCEALED_KONG)
	...
	def exposedKong(self, tile):
		...
		self.tiles.remove(tile)
		self.upTiles.append([tile, tile, tile, tile])
		self.upTiles_opList.append(const.OP_EXPOSED_KONG)
	...
	def continueKong(self, tile):
		...
		for i in range(len(self.upTiles)):
			...
			if meld[0] == meld[-1] and meld[0] == tile:
				...
				self.upTiles_opList[i] = const.OP_CONTINUE_KONG
	...
	def drawTile(self, tile, is_first = False):
		...
		if self.discard_state == const.DISCARD_FORCE:
			...
			if is_win:
				...
				self.owner.add_timer(1, Functor.Functor(self.forceDiscard, const.DISCARD_FORCE))
				# self.forceDiscard()
	...
	def forceDiscard(self, discard_state):
		if self.owner.can_discard(self.tiles, self.last_draw):
			DEBUG_MSG("forceDiscard:{0},{1}".format(self.idx, discard_state))
			self.owner.all_discard_tiles.append(self.last_draw)
			self.discardTile(self.last_draw, discard_state)
		else:
	...
	def discardTile(self, tile = None, discard_state = None):
		""" 打牌 """
		if tile is None:
			tile = self.last_draw
		...
		if self.owner.discard_seconds > 0:
			...
			if self.discard_state == const.DISCARD_FORCE or discard_state == const.DISCARD_FORCE:
				self.owner.broadcastOperation(self.idx, const.OP_DISCARD, [tile, ])
			else:
	...
	def get_round_client_dict(self):
		...
		return {
			...
			'upTiles': self.upTiles,
			'upTiles_opList': self.upTiles_opList,
			'score': self.score,
