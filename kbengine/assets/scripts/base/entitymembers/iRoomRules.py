...
class iRoomRules(object):
		...
		for t in lucky_tiles:
			...
			for i in range(len(const.LUCKY_TUPLE)):
				if t in const.LUCKY_TUPLE[i]:
					idx = i
					break
			sel_idx = (self.dealer_idx + idx) % 4

			DEBUG_MSG("cal_lucky_tile_score sel_idx:{0}, luckyTile:{1}".format(sel_idx, t))
			if sel_idx < 0 or sel_idx >= self.player_num:
				continue
			if self.player_num == 2 and sel_idx != self.dealer_idx:
				continue
			#杠分
			kong_base_score = 1
	...
	def tenPaiTiles(self, handTiles):
	...
	def can_change_discard_state(self, tiles, i, state):
		if state == const.DISCARD_FREE:
			return self.players_list[i].discard_state == const.DISCARD_FORCE
	...
	def can_win(self, handTiles, finalTile, win_op, idx):
		...
		# 天胡 地胡
		if isCanWin:
			if idx == self.dealer_idx and len(self.all_discard_tiles) <= 0 and win_op == const.OP_DRAW_WIN:
				result[12] = 1
				quantity = 40
			elif len(self.all_discard_tiles) == 1 and win_op == const.OP_GIVE_WIN:
				result[13] = 1
				quantity = 40
