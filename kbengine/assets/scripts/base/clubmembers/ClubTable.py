...
class Table(object):
	...
	def __init__(self, owner, club, table_index):
	...
	def takeASeat(self, avatar):
		...
		if self.roomAvailable():
			...
			def check_cb(result, msg=None):
				...
				self.room = KBEngine.createEntityLocally("GameRoom", params)
				if self.room:
	...
	def dismissRoom(self):
		if self.roomAvailable():
			...
			self.room.destroyByClub("房间已被亲友圈房主解散, 请重新加入游戏")
...
class TableManager(object):
	...
	def initTable(self):
		for i in range(const.CLUB_TABLE_NUM):
			self.tables[i] = Table(self, self.club, i)
