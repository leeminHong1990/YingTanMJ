...
class iClubOperation(object):
	...
	def createClub(self, club_name, create_dict):
		...
		if game_mode not in const.GAME_MODE \
				...
				or player_num not in const.PLAYER_NUM:
	...
	def getClubListInfo(self):
		club_info_list = []
		for club_id in self.clubList:
			d = x42.ClubStub.getClubAbstract(club_id, self.userId)
	...
	def pushClubEventToClient(self, club_id, event_id, data):
		...
		ID2FUNC = {
			Events.EVENT_OWNER_CHANGE				: 	'clubEvent_OC',	# 1
			Events.EVENT_DISMISS_ADD				:  	'clubEvent_DA',	# 2
			Events.EVENT_DISMISS_SUB				:  	'clubEvent_DS',	# 3
			Events.EVENT_DISMISS_CHANGE				:  	'clubEvent_DC',	# 4
		}
