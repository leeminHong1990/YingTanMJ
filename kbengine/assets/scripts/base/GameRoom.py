...
class GameRoom(BaseEntity, GameObject, iRoomRules):
	...
	def _reset(self):
		...
		if self.room_type == const.CLUB_ROOM and self.club and self.club_table:
			self.club.cancelDismiss(self.club_table.table_index)
		self.agent = None
		self.players_list = [None] * self.player_num
		self.dealer_idx = 0
		self.current_idx = 0
	...
	def apply_dismiss_room(self, avt_mb):
		...
		if self.club is not None:
			self.club.applyDismiss(self.club_table.table_index, self.roomID)
			for k, v in enumerate(self.players_list):
				if v is not None:
					v.mb.showTip("正在等待圈主确认是否允许中途退出")
			return
	...
	def do_drop_room(self):
		if self.game_result:
			if 'round_result' not in self.game_result or len(self.game_result['round_result']) == 0:
				self.dropRoom()
			else:
	...
	def broadcastRoundEnd(self, info):
		...
		info['player_round_list'] = [p.get_round_client_dict() for p in self.players_list if p is not None]
		info['player_info_list'] = [p.get_simple_client_dict() for p in self.players_list if p is not None]
	...
	def paySuccessCbk(self):
		...
		try:
			...
			if switch.DEBUG_BASE == 0:
				...
				prefabKingTiles = []
				# prefabHandTiles = [[1,1,1,2,2,2,3,3,3,4,4,4,5], [4,3,2,5,5,5,6,6,6,6], [], []]
				prefabHandTiles = [[], [], [], []]
				prefabTopList = []
	...
	def beginRound(self, is_first=False):
		...
		if p.userId in table_white.data:
			tenPaiTiles = self.tenPaiTiles(p.tiles)
			random.shuffle(tenPaiTiles)
			if len(tenPaiTiles) > 0:
				DEBUG_MSG("cheat tiles tenPaiTiles:{}".format(tenPaiTiles))
				for t in tenPaiTiles:
					if t in self.tiles:
						DEBUG_MSG("cheat tiles begin tiles:{}".format(self.tiles))
						self.tiles.remove(t)
						ts, self.tiles = self.tiles, [t]
						self.tiles.extend(ts)
						ti = self.tiles[0]
						self.tiles = self.tiles[1:]
						DEBUG_MSG("cheat tiles end tiles:{}".format(self.tiles))
	...
	def get_reconnect_room_dict(self, userId):
		...
		return {
			...
			'history_result': [self.history_result] if self.current_round > 1 else [],
		}
	...
	def destroyByClub(self, reason=None):
		# 此接口由GameWorld关服时调用
		if self.current_round > 0:
			for p in self.players_list:
				if p and p.mb:
