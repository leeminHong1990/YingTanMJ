...
var GameRoomUI = UIBase.extend({
	...
	initUI:function(){
		...
		for(var i=0; i<4; i++){
			var seat_list = const_val.PLAYER_SEAT[player.curGameRoom.player_num];
			if(seat_list && seat_list.indexOf(i) < 0){
                this.rootUINode.getChildByName("player_info_panel" + i.toString()).setVisible(false);
                this.rootUINode.getChildByName("player_tile_panel" + i.toString()).setVisible(false);
                this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel" + i.toString()).setVisible(false);
			}
		}
	...
	init_extra_panel:function(){
		...
		var extra_btn_2 = ccui.helper.seekWidgetByName(extra_operation_panel, "extra_btn_2")
		function extra_btn_1_event(sender, eventType){
			if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                //     h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, 'show_extra_panel', false, const_val.DISCARD_FORCE);
        ...
        function extra_btn_2_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                var player = h1global.player();
        ...
        extra_btn_2.addTouchEventListener(extra_btn_2_event);


        var player = h1global.player();
        var can_force_discard = false;
		if((player.curGameRoom.handTilesList[player.serverSitNum].length)%3==1){
            let canWinTiles = player.getCanWinTiles();
            can_force_discard = canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE ? true : false;
            this.show_extra_panel(can_force_discard, const_val.DISCARD_FORCE);
		}
		if(!can_force_discard){
            this.show_extra_panel(player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FORCE ? true : false, const_val.DISCARD_FREE);
		}
	},
	...
	force_discard:function () {
        ...
        if (h1global.curUIMgr.roomLayoutMgr) {
            ...
            h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, 'show_extra_panel', true, const_val.DISCARD_FREE);
            h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME,
                'update_player_hand_tiles',player.serverSitNum,player.curGameRoom.handTilesList[player.serverSitNum]);
        }
    },

	free_discard:function () {
        var player = h1global.player();
        this.touch_state = const_val.PLAYER_TOUCH_SELF_STATE;
        if (h1global.curUIMgr.roomLayoutMgr) {
            // h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, 'hide_operation_panel');
            if((player.curGameRoom.handTilesList[player.serverSitNum].length)%3===1){
                let canWinTiles = player.getCanWinTiles();
                h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, 'show_extra_panel', canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE ? true : false, const_val.DISCARD_FORCE);
            } else {
                h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, 'show_extra_panel', false, const_val.DISCARD_FORCE);
			}
            h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME,
	...
	show_extra_panel:function(is_show, discard_type){
		this.rootUINode.getChildByName("extra_operation_panel").setVisible(is_show);

        this.rootUINode.getChildByName("extra_operation_panel").getChildByName("extra_btn_1").setVisible(discard_type === const_val.DISCARD_FORCE);
        this.rootUINode.getChildByName("extra_operation_panel").getChildByName("extra_btn_2").setVisible(discard_type === const_val.DISCARD_FREE);
	},
	...
	init_curplayer_panel:function(){
		...
		// this.show_extra_panel(canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE  ? true : false, const_val.DISCARD_FORCE);
	...
	update_curplayer_panel:function(serverSitNum){
		...
		for(var i = 0; i < 4; i++){
			...
			if(seat_list.indexOf(i) >= 0 && i == player.server2CurSitNum(serverSitNum)){
	...
	mark_tips_tile:function (tile_img) {
        tile_img.color = const_val.mark_tips_color;
    },

    cancel_tips_tile:function(tile_img){
        tile_img.color = const_val.mark_none_color;
    },
		...
		function playAnimation(){
					...
					cc.CallFunc.create(function(){
						...
						if(cur_tile_num == groupSize){
							for(var i = 0; i < player.curGameRoom.player_num; i++){
								...
								var cur_player_tile_panel = self.rootUINode.getChildByName("player_tile_panel" + index);
			...
			var step2 = cc.Sequence.create(cc.DelayTime.create(0.1),cc.CallFunc.create(function(){
				...
				for (var i = 0; i < player.curGameRoom.player_num; i++) {
					...
					tile_down_anim_node.scale = options.downRootNodeScales[index];
					tile_down_anim_node.setPosition(options.downRootNodeOffsets[index])
					for(var j = 0; j < 13; j++){
						if(options.tileTopAndDownImgPaths){
                            img = ccui.ImageView.create(options.tileTopAndDownImgPaths[index].length == 0 ? options.tileDownImgPaths[index] : cc.formatStr(options.tileDownImgPaths[index], options.tileTopAndDownImgPaths[index][j]) ,ccui.Widget.PLIST_TEXTURE);
                        }else{
                            img = ccui.ImageView.create(options.tileDownImgPaths[index] ,ccui.Widget.PLIST_TEXTURE);
						}
						if((index == 1 || index == 3) && options.tilescale){
                            img.setScale(1 / (options.tilescale[j] * 0.01));
						}
						img.setPosition(options.downTilePositionFuncs[index](j))
						img.setAnchorPoint(0,0)
                        if(options.downRootNodeAnchorPointX){
                            img.setAnchorPoint(options.downRootNodeAnchorPointX[index],0);
                        }
                        if(options.downRootNodeFlippedX){
                            img.setFlippedX(options.downRootNodeFlippedX[index]);
                        }
						if(options.downRootNodeRotations){
                            img.setRotation(options.downRootNodeRotations[index]);
                        }
			...
			var step3 = cc.CallFunc.create(function(){
				...
				let canWinTiles = player.getCanWinTiles();
				self.show_extra_panel(canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE ? true : false, const_val.DISCARD_FORCE);
                cc.audioEngine.playEffect("res/sound/effect/deal_tile.mp3");
			});
	...
	stopBeginAnim:function(){
		...
		for(var j = 0; j < player.curGameRoom.player_num; j++){
            var index = player.server2CurSitNum(j);
			this.update_player_hand_tiles(j);
            var cur_player_tile_panel = this.rootUINode.getChildByName("player_tile_panel" + index);
            cur_player_tile_panel.setVisible(true);
			var tile_down_anim_node = cur_player_tile_panel.getChildByName("tile_down_anim_node");
			if(tile_down_anim_node){
				tile_down_anim_node.removeFromParent();
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        var tips_tiles = player.getDiscardTipsTiles();
                ...
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
                        if(discardState === const_val.DISCARD_FORCE){
                        	...
                        	this.cancel_same_tile(tile_img);
                            if(tips_tiles.indexOf(handTilesList[i]) >= 0){
                                this.mark_tips_tile(tile_img);
                            } else {
                                this.cancel_tips_tile(tile_img)
                            }
						}
                    }
	...
	update_discard_tips_tile:function () {
		if(!this.is_show){return;}
        var player = h1global.player();
        var tips_tiles = [];
		if(player.curGameRoom.curPlayerSitNum === player.serverSitNum){
			tips_tiles = player.getDiscardTipsTiles()
		}
	...
	play_luckytiles_anim:function(luckyTileList, callback){
        ...
        show_lucky_tiles_panel.runAction(cc.Sequence.create(cc.MoveTo.create(1.0, cc.p(cc.winSize.width/2, cc.winSize.height/2)), cc.CallFunc.create(function () {
            for(var i = 0; i < 2; i++){
                var tile_img = show_lucky_tiles_panel.getChildByName("tile_img" + i.toString());
                var mahjong_img = tile_img.getChildByName("mahjong_img");
                var luckyTileNum = luckyTileList[i];
		...
		show_lucky_tiles_panel.runAction(cc.Sequence.create(cc.DelayTime.create(luckyTileList.length * 0.4 + 1.8), cc.CallFunc.create(function(){
			if(callback){callback();}
		})));
	},

    play_result_anim: function (player_round_list) {
        var player = h1global.player();
        var curGameRoom = player.curGameRoom;
        for (var i = 0; i < curGameRoom.player_num; i++) {
            var player_info = player_round_list[i];
