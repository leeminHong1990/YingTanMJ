...
var GameRoom2DUI = GameRoomUI.extend({
    ...
    init_curplayer_panel_direction:function(){
        ...
        var interval = (seat_list[player.serverSitNum] - seat_list[player.curGameRoom.dealerIdx] + 4) % 4;
        var player_panel = this.cur_player_panel.getChildByName("player_panel");
        player_panel.setRotation((90 * interval) % 360);
    },
    ...
    update_curplayer_panel: function (serverSitNum) {
        ...
        var player_interval = (seat_list[player.serverSitNum] - seat_list[player.curGameRoom.dealerIdx] + 4) % 4;

        var toward_index = serverSitNum;
        if (player.curGameRoom.player_num === 2){
            // 二人 面对面 0/1 ==>> 0/2
            toward_index = serverSitNum*2
        } else if(player.curGameRoom.player_num === 3){
            // 三个人的时候 需要跳过一个格子
            var skip = {0:2, 1:3, 3:1};
            toward_index = serverSitNum >= skip[player_interval] ? (serverSitNum +1)%4 : serverSitNum
        }
