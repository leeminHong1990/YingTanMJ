...
var GameRoomPrepareUI = UIBase.extend({
	...
	initUI:function(){
		...
		var share_title = '鹰潭麻将 ' + (player.curGameRoom.club_id > 0 ? '亲友圈号【' + player.curGameRoom.club_id.toString() + '】' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
    ...
    init_player_info_panel:function () {
        ...
        var gameprepare_panel = this.rootUINode.getChildByName("gameprepare_panel");

        var px = {
            2:[0.4, 0.6],
            3:[0.35,0.5,0.65],
            4:[0.3,0.433,0.566,0.7]
        };
        for (var i = 0; i < 4; i++) {
            if(i >= player.curGameRoom.player_num){
                gameprepare_panel.getChildByName("frame_img_"+ i.toString()).setVisible(false);
                gameprepare_panel.getChildByName("player_info_panel" + i.toString()).setVisible(false);
            }else {
                gameprepare_panel.getChildByName("frame_img_"+ i.toString()).setPositionX(cc.winSize.width*px[player.curGameRoom.player_num][i]);
                gameprepare_panel.getChildByName("player_info_panel" + i.toString()).setPositionX(cc.winSize.width*px[player.curGameRoom.player_num][i]);
            }
        }
    },
