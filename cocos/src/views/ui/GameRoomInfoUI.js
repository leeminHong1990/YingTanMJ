var GameRoomInfoUI = MultipleLayoutUI.extend({
	...
	updateLayout2D: function() {
		...
		this.history_btn = this.function_panel.getChildByName("history_btn");
	...
	updateLayout: function() {
		...
		this.history_btn.addTouchEventListener(function (sender, eventType) {
			if(eventType === ccui.Widget.TOUCH_ENDED){
				var player = h1global.player();
				if(player && player.curGameRoom && h1global.curUIMgr.settlement_ui && !h1global.curUIMgr.settlement_ui.is_show){
                    h1global.curUIMgr.settlement_ui.show_by_info(player.curGameRoom.history_result, player.serverSitNum, undefined, undefined, true)
				}
			}
        });
	...
	update_history:function () {
        if(h1global.player() && h1global.player().runMode !== const_val.GAME_ROOM_PLAYBACK_MODE && h1global.player().curGameRoom.curRound > 1){
            this.history_btn.setVisible(true)
        } else {
            this.history_btn.setVisible(false)
        }
    },
