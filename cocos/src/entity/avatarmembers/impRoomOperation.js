...
var impRoomOperation = impGameRules.extend({
	...
	server2CurSitNum : function(serverSitNum){
		if(this.curGameRoom){
			var seat_list = const_val.PLAYER_SEAT[this.curGameRoom.player_num];
			return seat_list[(serverSitNum + this.curGameRoom.player_num - this.serverSitNum) % this.curGameRoom.player_num];
		} else {
			return -1;
		}
	},
	...
	applyDismissRoom : function(){
		if(this.curGameRoom){
			...
			if(this.curGameRoom.club_id > 0){
                h1global.globalUIMgr.info_ui.show_by_info("已发送给群主,请等待群主审核", cc.size(300, 200));
				return
			}
