...
var impGameRules = impGameOperation.extend({
    ...
    getDiscardTipsTiles: function () {
        var handTiles = this.curGameRoom.handTilesList[this.serverSitNum].concat([]);
        if(handTiles.length%3 !== 2){return [];}
        ...
        for(var tile in tile2NumDict){
            var s_tiles = handTiles.concat([]);
            s_tiles.splice(s_tiles.indexOf(parseInt(tile)), 1);

            for (var i = 0; i < allTiles.length; i++) {
                var can_win = false;
                for (var j = 0; j < allTiles[i].length; j++) {
                    var t = allTiles[i][j];
                    var temp_handTiles = s_tiles.concat([t]);
                    if (this.canWin(temp_handTiles)) {
                        tips_tiles.push(parseInt(tile));
                        can_win = true;
                        break
                    }
                }
