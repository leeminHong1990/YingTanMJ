...
var impGameOperation = impCommunicate.extend({
	...
	startGame : function(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList){
		...
		if(h1global.curUIMgr.gameroominfo_ui && h1global.curUIMgr.gameroominfo_ui.is_show){
            ...
            h1global.curUIMgr.gameroominfo_ui.update_history();
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
					...
					var can_discard_force = canWinTiles.length > 0 && this.curGameRoom.discardStateList[this.serverSitNum] == const_val.DISCARD_FREE ? true : false;
					if(can_discard_force){
                        h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "show_extra_panel", can_discard_force, const_val.DISCARD_FORCE);
					}else if(this.curGameRoom.discardStateList[this.serverSitNum] === const_val.DISCARD_FORCE){
    ...
    //                 h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_operation_panel", this.getPongKongOpDict(), const_val.SHOW_DO_OP);
			...
			if(this.serverSitNum == serverSitNum){
				...
				this.curGameRoom.upTilesList[serverSitNum].push([0,0,0,0]);
	...
	postPlayerDiscardState : function (idx, state){
        ...
        if(idx === this.serverSitNum){
            if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                if(state === const_val.DISCARD_FORCE){
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "force_discard");
				} else {
                    h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "free_discard");
				}
            }
        } else {
            if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
	...
	roundResult : function(roundRoomInfo){
		...
		for(var i = 0; i < player_round_list.length; i++){
			this.curGameRoom.playerInfoList[i]["score"] = player_round_list[i]["score"];
			this.curGameRoom.playerInfoList[i]["total_score"] = player_round_list[i]["total_score"];
		}
		var anim_end_num = 0;
        var self = this;
        ...
        if(self.runMode === const_val.GAME_ROOM_PLAYBACK_MODE){
        	...
        	this.curGameRoom.history_result = roundRoomInfo
		}
		...
		function callbackfunc(){
            if (self.runMode === const_val.GAME_ROOM_PLAYBACK_MODE) {
                h1global.curUIMgr.settlement_ui.show_by_info(roundRoomInfo, serverSitNum, undefined, replay_func);
            } else {
                if (anim_end_num >= 2 && h1global.curUIMgr.settlement_ui) {
                    h1global.curUIMgr.settlement_ui.show_by_info(roundRoomInfo, serverSitNum);
                }
            }
		}
		function showResult() {
			if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
				h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "play_result_anim", player_round_list);
	...
	finalResult : function(finalPlayerInfoList, roundRoomInfo){
		...
		function showResult() {
			if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
				h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "play_result_anim", roundRoomInfo["player_round_list"]);
				h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "play_luckytiles_anim", roundRoomInfo["lucky_tiles"], function(){
					anim_end_num += 1;
					callbackfunc()
				});
